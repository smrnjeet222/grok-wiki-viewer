...
import type { WikiPageMeta, WikiRecord } from "./types";
...
export function pageMarkdown(wiki: WikiRecord, meta: WikiPageMeta, index: number): string {
  ...
  const sources = (meta.filePaths || []).map((f) => `- \`${f}\``).join("\n");
...
export function wikiListItemFromRecord(wiki: WikiRecord, path: string) {
...
export function isWikiRecord(value: unknown): value is WikiRecord {
...
export function openInBrowser(data: string, mime = "text/plain;charset=utf-8"): void {
  const url = URL.createObjectURL(new Blob([data], { type: mime }));
  window.open(url, "_blank", "noopener");
  window.setTimeout(() => URL.revokeObjectURL(url), 60_000);
}

export function triggerDownload(filename: string, data: string | Uint8Array, mime: string): void {
  const blob =
    typeof data === "string"
